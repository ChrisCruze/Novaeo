...
const columnsFromDictionaryProducts = ({ workbook }) => {
	const checkKey = (key_name) => Object.keys(workbook).indexOf(key_name) > -1;
...
const DataPortalConfig = () => {
	...
	const tableData = partsDataProcess({ workbook });
	const fieldsArray = fieldsFromDictionary({ workbook });
	productDictFromURLUpdate({ tableData, pageData, setPageData });
	const productsTableData = productsDataProcess({ workbook });
	const productsTableDataFiltered = productsTableData.filter((D) => {
		return D["part_id"].indexOf(queryDictFromURLParams()["part_id"]) > -1;
	});
	console.log({
		productsTableData,
		productsTableDataFiltered,
		workbook,
		tableData,
		fieldsArray,
		pageData,
			...
			},
			{
				...
				datatables: {
					...
					config: {
						...
						data: productsTableDataFiltered,
						columns: columnsFromDictionaryProducts({ workbook }),
						select: true,
						paging: false,
						scrollX: true,
						colReorder: true,
